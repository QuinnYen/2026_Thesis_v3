"""
Task 2.4: 訓練 Aspect Post-Fusion 模型（清理後資料）

配置:
- 模型: Post-Fusion (BiLSTM → Attention)
- 層數: 2, 3, 4, 5 層全部訓練
...
from experiment_config import DATASETS, POSTFUSION_CONFIG, get_output_dir, get_dataset_path
...
def train_model(num_layers, train_loader, val_loader, class_weights, device, base_output_dir):
    ...
    print("\n" + "=" * 80)
    print(f"訓練 Post-Fusion {num_layers} 層模型")
    print("=" * 80)
    ...
    # 輸出目錄
    OUTPUT_DIR = base_output_dir / f"post_fusion_{num_layers}layer_cleaned"
    ...
    vocab_size = sample_batch['input_ids'].max().item() + 100  # 加 buffer
    ...
    return final_f1, final_acc, total_params


def main():
    parser = argparse.ArgumentParser(description='訓練 Post-Fusion 模型')
    ...
    if args.layers is None:
        args.layers = POSTFUSION_CONFIG.get('layers', [2, 3, 4, 5])

    print("=" * 80)
    print("Task 2.4: 訓練 Aspect Post-Fusion 模型（清理後資料）")
    print("=" * 80)
    ...
    train_loader, val_loader, vocab, label_map, class_weights = create_cleaned_data_loaders(
        train_csv=CSV_PATH,
        batch_size=POSTFUSION_CONFIG.get('batch_size', 32),
        val_split=0.2,
        max_length=POSTFUSION_CONFIG.get('max_length', 100),
        min_freq=2,
        random_seed=42
    )

    base_output_dir = get_output_dir(dataset_name, "post_fusion").parent
    ...
    for num_layers in args.layers:
        f1, acc, params = train_model(num_layers, train_loader, val_loader, class_weights, device, base_output_dir)
        all_results[num_layers] = {'f1': f1, 'accuracy': acc, 'params': params}

    # 輸出總結
    print("\n" + "=" * 80)
    print("訓練總結")
    print("=" * 80)
    print(f"\n{'層數':<10} {'參數量':<15} {'Accuracy':<15} {'Macro-F1':<15}")
    print("-" * 55)
    for num_layers in sorted(all_results.keys()):
        res = all_results[num_layers]
        print(f"{num_layers:<10} {res['params']:<15,} {res['accuracy']:<15.4f} {res['f1']:<15.4f}")
